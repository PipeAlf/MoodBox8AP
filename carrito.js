...
async function fetchCarritoDelServidor(usuarioId, token) {
  try {
    const response = await fetch(`https://8mq33rknsp.us-east-1.awsapprunner.com/api/carrito/${usuarioId}`, {
...
async function addToCart(product) {
  ...
  try {
    ...
    const response = await fetch("https://8mq33rknsp.us-east-1.awsapprunner.com/api/carrito", {
      method: "POST",
      headers: {
...
async function removeItem(itemId) {
  const token = localStorage.getItem("accessToken");
  await fetch(`https://8mq33rknsp.us-east-1.awsapprunner.com/api/carrito/${itemId}`, {
    method: "DELETE",
    headers: { "Authorization": `Bearer ${token}` }
  });
